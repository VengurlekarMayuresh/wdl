...
mongoose.connect('mongodb+srv://onlinemayuresh29_db_user:JSd35Yqxu4whtzNH@wdl.zvk3ngr.mongodb.net/healthcare_db');
...
async function rebuildAllAccounts() {
    try {
        ...
        for (const doctorData of doctorsData) {
            try {
                // Hash password manually (disable pre-save middleware)
                const hashedPassword = await bcrypt.hash(doctorData.password, 10);
                
                // Create user record directly via insertOne to bypass pre-save middleware
                const userData = {
                ...
                const userResult = await User.collection.insertOne(userData);
