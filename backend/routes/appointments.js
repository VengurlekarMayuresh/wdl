...
router.post("/slots", authenticate, authorize("doctor"), async (req, res) => {
  try {
    ...
    if (
      ...
      console.log(
        "🔧 Development mode: Creating mock doctor profile for slots"
      );
      doctor = await Doctor.create({
        userId: req.user._id,
        medicalLicenseNumber: "DEV123456",
...
router.get("/slots/doctor/:doctorId", async (req, res) => {
  try {
    ...
    const slots = await Slot.find(query).sort({ dateTime: 1 }).limit(50); // Limit to prevent too many results
...
router.post("/", authenticate, authorize("patient"), async (req, res) => {
  try {
    ...
    if (slotId) {
      ...
      // Notify both parties
      try {
        await emailService.notifyAppointmentConfirmed(appointment);
      } catch {}
    ...
    if (doctorId && requestedDateTime) {
      ...
      try {
        await emailService.notifyAppointmentRequested(appointment);
      } catch {}
      ...
      return res.status(201).json({
        success: true,
        message:
          "Appointment request sent successfully! The doctor will review your request.",
        data: { appointment: populatedAppointment },
      });
    }
    ...
    return res.status(400).json({
      success: false,
      message:
        "Provide either a valid slotId to book immediately or doctorId and requestedDateTime to request an appointment.",
    });
  } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        console.log(
          "🔧 Development mode: Creating mock doctor profile for appointments"
        );
        doctor = await Doctor.create({
          userId: req.user._id,
          medicalLicenseNumber: "DEV123456",
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const appointments = await Appointment.findForPatient(
        patient._id,
        parseInt(limit)
      ).populate({
        path: "doctorId",
  ...
  try {
    ...
    if (req.user.userType !== "doctor") {
      return res
        ...
        .json({
          success: false,
          message: "Access denied. Required role: doctor or patient",
        });
    }
    ...
    switch (status) {
          ...
          if (isNaN(targetDate.getTime()) || targetDate <= new Date()) {
            return res
              ...
              .json({
                success: false,
                message:
                  "Cannot confirm: appointment date/time is invalid or in the past",
              });
          }
      ...
      case "rejected":
        ...
        if (slot) {
          await slot.cancelBooking(
            "doctor",
            reason || "Appointment cancelled/rejected"
          );
        }
        try {
          await emailService.notifyAppointmentStatus(appointment, status, {
            reason,
          });
        } catch {}
        break;
      case "completed":
        ...
        try {
          await emailService.notifyAppointmentStatus(appointment, "completed");
        } catch {}
        break;
      default:
  ...
  try {
    ...
    if (req.user.userType !== "patient") {
      return res.status(403).json({
        success: false,
        message:
          "Only patients can directly reschedule. Doctors should use the propose workflow.",
      });
    }
    ...
    console.log(
      `✅ Updated existing slot ${currentSlot._id
        .toString()
        .substr(-8)} with new timing`
    );
    ...
    try {
      ...
      console.log(
        `🗑️ Deleted temporary slot ${newSlot._id
          .toString()
          .substr(-8)} after copying details`
      );
    } catch (deleteError) {
      console.warn(
        `⚠️ Could not delete temporary slot ${newSlot._id}: ${deleteError.message}`
      );
    }
    ...
    try {
      await emailService.notifyAppointmentStatus(appointment, "rescheduled");
    } catch {}
    ...
    res.json({
      success: true,
      message:
        "Appointment rescheduled successfully. Your appointment has been confirmed for the new time.",
      data: {
...
async function cleanupDuplicateRescheduleAppointments(appointment) {
  try {
    console.log(
      `🧹 Starting comprehensive cleanup for appointment ${appointment._id
        .toString()
        .substr(-8)}`
    );
    ...
    const reschedulePatterns = [
      ...
      new RegExp(
        `doctor reschedule request.*${appointment._id.toString()}`,
        "i"
      ),
      new RegExp(
        `patient reschedule request.*${appointment._id.toString()}`,
        "i"
      ),
      new RegExp(
        `reschedule request for appointment ${appointment._id.toString()}`,
        "i"
      ),
    ];
    ...
    if (duplicateAppointments.length > 0) {
      console.log(
        `🗑️ Found ${duplicateAppointments.length} potential duplicate appointments`
      );

      // Log details of duplicates for debugging
      duplicateAppointments.forEach((dup) => {
        console.log(
          `  - ${dup._id.toString().substr(-8)}: ${dup.reasonForVisit.substring(
            0,
            50
          )}... (${dup.status})`
        );
      ...
      console.log(
        `🗑️ Deleted ${deleteResult.deletedCount} duplicate appointments`
      );
      ...
      if (duplicateSlotIds.length > 0) {
        ...
        console.log(
          `🗑️ Deleted ${slotDeleteResult.deletedCount} orphaned duplicate slots`
        );
      }
      ...
      return {
        ...
        slotsDeleted:
          duplicateSlotIds.length > 0 ? slotDeleteResult?.deletedCount || 0 : 0,
      };
    } else {
...
async function performGlobalRescheduleCleanup() {
  try {
    ...
    if (oldRescheduleRequests.length > 0) {
      console.log(
        `🗑️ Found ${oldRescheduleRequests.length} old reschedule requests to clean up`
      );
      ...
      console.log(
        `✅ Cleaned up ${oldRescheduleRequests.length} old reschedule requests`
      );
    }
    ...
    if (oldUnbookedSlots.length > 0) {
      ...
      console.log(
        `✅ Cleaned up ${oldUnbookedSlots.length} old unbooked slots`
      );
    }
...
// @access  Private (Doctor or Patient)
router.post(
  "/:appointmentId/reschedule/propose",
  authenticate,
  async (req, res) => {
    try {
      ...
      if (req.user.userType === "doctor") {
        ...
        owns =
          ...
          proposerProfile._id.equals(appointment.doctorId._id);
      } else if (req.user.userType === "patient") {
        proposerProfile = await Patient.findOne({ userId: req.user._id });
        owns =
          proposerProfile &&
          appointment.patientId &&
          proposerProfile._id.equals(appointment.patientId._id);
      }
      if (!owns) {
        return res
          .status(403)
      ...
      if (!proposedSlotId && !proposedDateTime) {
        return res
          ...
          .json({
            success: false,
            message: "Provide proposedSlotId or proposedDateTime",
          });
      }
      ...
      if (proposedSlotId) {
        ...
        if (!proposedSlot) {
          return res
            .status(404)
            .json({ success: false, message: "Proposed slot not found" });
        }
        if (!proposedSlot.canBeBooked()) {
          return res
            ...
            .json({
              success: false,
              message: "Proposed slot is not available",
            });
        }
        ...
        if (!proposedSlot.doctorId.equals(appointment.doctorId._id)) {
          return res
            ...
            .json({
              success: false,
              message: "Slot must belong to the same doctor",
            });
        }
        ...
        if (isNaN(dt.getTime()) || dt <= new Date()) {
          return res
            ...
            .json({
              success: false,
              message: "Proposed date/time must be a valid future date",
            });
        }
        ...
        if (req.user.userType === "doctor") {
          ...
          if (existingSlot && !existingSlot.canBeBooked()) {
            return res
              ...
              .json({
                success: false,
                message:
                  "There is already a slot at the proposed time but it is not available",
              });
          }
          ...
          if (!existingSlot) {
            ...
            const baseDuration =
              appointment.slotId?.duration || appointment.duration || 30;
            const baseConsultationFee =
              appointment.doctorId?.consultationFee ?? undefined;
            existingSlot = await Slot.create({
              doctorId: appointment.doctorId._id,
              dateTime: normalizedDateTime,
              duration: baseDuration,
              consultationFee:
                baseConsultationFee !== undefined ? baseConsultationFee : 0,
              consultationType: appointment.consultationType || "in-person",
      ...
      console.log(
        `✅ ${
          req.user.userType
        } proposed reschedule for appointment ${appointment._id
          .toString()
          .substr(-8)}`
      );
      ...
      try {
        await emailService.notifyRescheduleProposed(
          appointment,
          normalizedDateTime,
          req.user.userType
        );
      ...
      res
        .status(500)
        .json({ success: false, message: "Server error proposing reschedule" });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (!pr.active) {
        return res
          .status(400)
          .json({ success: false, message: "No pending reschedule to decide" });
      }
      ...
      if (req.user.userType !== counterparty) {
        return res
          ...
          .json({
            success: false,
            message: "Only the other party can decide on this proposal",
          });
      }
      ...
      if (req.user.userType === "doctor") {
        ...
        canDecide =
          deciderProfile &&
          appointment.doctorId &&
          deciderProfile._id.equals(appointment.doctorId._id);
      } else if (req.user.userType === "patient") {
        deciderProfile = await Patient.findOne({ userId: req.user._id });
        canDecide =
          deciderProfile &&
          appointment.patientId &&
          deciderProfile._id.equals(appointment.patientId._id);
      }
      if (!canDecide) {
        return res
          .status(403)
          .json({
            success: false,
            message: "Not authorized to decide on this appointment",
          });
      }
      ...
      if (decision === "rejected") {
        console.log(
          `❌ ${
            req.user.userType
          } rejected reschedule for appointment ${appointment._id
            .toString()
            .substr(-8)}`
        );
        ...
        appointment.pendingReschedule.decisionAt = new Date();
        appointment.pendingReschedule.decisionReason =
          reason || "Reschedule rejected";
        appointment.lastModifiedBy = req.user.userType;

        await appointment.save();

        try {
          await emailService.notifyRescheduleDecision(
            appointment,
            "rejected",
            reason
          );
        ...
        return res.json({
          success: true,
          message:
            "Reschedule proposal has been rejected. The original appointment remains unchanged.",
      ...
      console.log(
        `✅ ${
          req.user.userType
        } approved reschedule for appointment ${appointment._id
          .toString()
          .substr(-8)}`
      );
      ...
      if (pr.proposedSlotId) {
        ...
        if (!proposedSlot || !proposedSlot.canBeBooked()) {
          return res
            ...
            .json({
              success: false,
              message: "Proposed slot is no longer available",
            });
        }
        ...
        try {
          ...
          console.log(
            `🗑️ Deleted proposed slot ${proposedSlot._id
              .toString()
              .substr(-8)} after approval`
          );
        } catch (deleteError) {
          console.warn(
            `⚠️ Could not delete proposed slot: ${deleteError.message}`
          );
        }
      } else if (!targetDate) {
        return res
          .status(400)
          .json({ success: false, message: "No proposed date to approve" });
      }
      ...
      if (conflictingAppointment) {
        return res.status(400).json({
          success: false,
          message:
            "Doctor already has another appointment at the approved time",
        });
      }
      ...
      if (targetSlotDetails.duration)
        currentSlot.duration = targetSlotDetails.duration;
      if (targetSlotDetails.consultationFee)
        currentSlot.consultationFee = targetSlotDetails.consultationFee;
      if (targetSlotDetails.consultationType)
        currentSlot.consultationType = targetSlotDetails.consultationType;

      await currentSlot.save();
      console.log(
        `✅ Updated existing slot ${currentSlot._id
          .toString()
          .substr(-8)} with approved timing`
      );

      // UPDATE the appointment with new timing and status
      appointment.appointmentDate = targetDate;
      if (targetSlotDetails.duration)
        appointment.duration = targetSlotDetails.duration;
      if (targetSlotDetails.consultationFee)
        appointment.consultationFee = targetSlotDetails.consultationFee;
      ...
      try {
        await emailService.notifyRescheduleDecision(appointment, "approved");
      } catch {}
      ...
      const approverType =
        req.user.userType === "doctor" ? "doctor" : "patient";
      ...
      res
        ...
        .json({
          success: false,
          message: "Server error processing reschedule decision",
        });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        console.log(
          "🔧 Development mode: Creating mock doctor profile for patients"
        );
        doctor = await Doctor.create({
          userId: req.user._id,
          medicalLicenseNumber: "DEV123456",
      ...
      appointments.forEach((appointment) => {
        ...
        if (appointmentDate < now) {
          ...
          if (
            ...
            appointmentDate >
              new Date(patientData.stats.lastAppointment.appointmentDate)
          ) {
            patientData.stats.lastAppointment = {
              _id: appointment._id,
          ...
          if (
            ...
            appointmentDate <
              new Date(patientData.stats.nextAppointment.appointmentDate)
          ) {
            patientData.stats.nextAppointment = {
              _id: appointment._id,
              appointmentDate: appointment.appointmentDate,
    ...
    try {
      ...
      if (agg.length > 0) {
        ...
        await Doctor.findByIdAndUpdate(
          ...
          { averageRating: avg, totalReviews: total },
          { new: true }
        );
      }
